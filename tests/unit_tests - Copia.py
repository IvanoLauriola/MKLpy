...
class TestGenerators(unittest.TestCase):
	...
	def test_multiview(self):
		XLtr = [self.Xtr + i for i in range(5)]
		XLte = [self.Xte + i for i in range(5)]
		kf   = lambda _X, _Z : pairwise_mk.homogeneous_polynomial_kernel(_X, _Z, degree=2)
		KLtr = [kf(X, X)  for X in XLtr]
		KLte = [kf(Xt, X) for Xt,X in zip(XLte, XLtr)]

		KLtr_g = Multiview_generator(XLtr, kernel=kf)
		KLte_g = Multiview_generator(XLte, XLtr, kernel=kf)
		print (XLtr[0].shape, XLte[0].shape, average(KLtr).size(), average(KLte).size(), average(KLtr_g).size(), average(KLte_g).size())
		self.assertTrue(matNear(average(KLtr), average(KLtr_g)))
		self.assertTrue(matNear(average(KLte), average(KLte_g)))
		self.assertTrue(matNear(KLtr[1], KLtr_g[1]))
		self.assertTrue(matNear(KLte[0], KLte_g[0]))
