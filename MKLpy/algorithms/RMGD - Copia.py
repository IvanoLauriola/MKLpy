...
class RMGD(BaseEstimator, ClassifierMixin, MKL):
    ...
    def __init__(self, estimator=SVC(), step=0.01, generator=HPK_generator(n=10), multiclass_strategy='ova', max_iter=200, tol=0.00001, verbose=False):
    ...
    def _arrange_kernel(self):
        ...
        for i in xrange(self.max_iter):
            ...
            new_ratio = _r**2/_m**2
            if actual_ratio and abs(new_ratio - actual_ratio) < self.tol:
                #completato
                break;
            ...
            new_eta = [cstep * (num[k]/den[k]) + eta[k] for k in range(nk)]
            new_eta = [max(0,v) for v in new_eta]
            eta = np.array(new_eta)/sum(new_eta)
