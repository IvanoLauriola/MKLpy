...
class EasyMKL(BaseEstimator, ClassifierMixin, MKL):
    ...
    def __init__(self, estimator=KOMD(lam=0.1), lam=0.1, generator=HPK_generator(n=10),
                 multiclass_strategy='ova', max_iter=100, verbose=False):
    ...
    def _arrange_kernel(self):
        ...
        A = matrix(
            [[1.0 if lab == +1 else 0 for lab in Y], [1.0 if lab2 == -1 else 0 for lab2 in Y]]).T
