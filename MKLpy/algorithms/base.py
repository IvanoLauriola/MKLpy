...
class MKL(object):
    ...
    def fit(self, X, Y):
        ...
        if self.multiclass_:
            metaClassifier = OneVsOneMKLClassifier if self.multiclass_strategy in ['ovo', 'OvO',
                                                                                   '1v1'] else OneVsRestMKLClassifier
    ...
    def predict(self, X):
        ...
        return self.clf.predict(KL) if self.multiclass_ else self.estimator.predict(
...
class AverageMKL(BaseEstimator, ClassifierMixin, MKL):
    ...
    def __init__(self, estimator=SVC(C=1), generator=HPK_generator(n=10), multiclass_strategy='ova',
                 max_iter=100, verbose=False):
        super(self.__class__, self).__init__(estimator=estimator, generator=generator,
                                             multiclass_strategy=multiclass_strategy,
                                             how_to=average, max_iter=max_iter, verbose=verbose)
